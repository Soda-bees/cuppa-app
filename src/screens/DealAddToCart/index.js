...
import {addDealToCart, addToCart} from '../../services/config/API';
import {selectUserData, setUserData} from '../../store/userDetails';
import Modal from 'react-native-modal';
import {selectOutlets} from '../../store/outletsSlice';

export default function DealAddToCart({navigation, route}) {
  const {menus, deal} = route.params;
  console.log(deal);
  ...
  const [item, setItem] = useState(menus[index]);
  const [cartItemsArray, setCartItemsArray] = useState([]);

  const allCafes = useSelector(selectOutlets);

  const cafe = allCafes.find(cafea => cafea._id == item?.cafeId);
  ...
  const handleAddToCart = async () => {
    try {
      ...
      const updatedCartItemsArray = [...cartItemsArray, newCartItem];

      console.log('deaaaaaaaallllllllllll', updatedCartItemsArray);

      if (!(index < menus.length - 1)) {
        setLoader(true);

        const body = {
          cartItems: updatedCartItemsArray,
        };

        console.log(body);

        const response = await addDealToCart(body, token);
        ...
        setCartItemsArray(updatedCartItemsArray);
  ...
  return (
    <SafeAreaView>
      ...
      <View style={styles.mainContainer}>
        <Header iconType={'teal'} favourite={false} title={item?.name} />
