...
export default function AdminEditOutlet() {
  ...
  const [outletName, setOutletName] = useState('Gloria Jeans Manhattan');
  const [description, setDescription] = useState(`gloria jeans is one of the world's biggest premium coffee chains`);
  const [modalShow, setModalShow] = useState(false);
  const [userImage, setUserImage] = useState('');
  const [selectedCountry, setSelectedCountry] = useState('Manhattan, New York');
